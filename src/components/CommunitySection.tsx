...
import { db, collection, query, orderBy, onSnapshot, addDoc, auth } from '../lib/firebase';
import { CommunityPost, BlogPost } from '../types';
        ...
        {activeTab === 'gallery' ? (
          ...
          >
            ...
            <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
              {posts.map(post => (
                <div key={post.id} className="break-inside-avoid glass rounded-2xl overflow-hidden border border-white/5">
                  <img src={post.imageUrl} alt={post.caption || 'Community foto'} className="w-full h-auto object-cover" />
                  <div className="p-4 space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-[10px] font-mono text-white/40 uppercase">
                        <span className="text-white/80">{post.userName}</span>
                        <span className="mx-2">•</span>
                        {format(parseISO(post.timestamp), 'd MMM', { locale: nl })}
                      </div>
                    </div>
                  </div>
                </div>
